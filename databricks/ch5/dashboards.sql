...
max(max_workers) as max_workers, sum(max_workers) as total_workers, CAST(avg(max_workers) as UNSIGNED) as avg_workers
from
(
	SELECT jr.job_run_name, c.node_type_id,
		jr.job_start_time_dt,
...
max(avg_workers) as max_of_avg_workers, max(total_workers) as max_of_total_workers
FROM
(
	SELECT node_type_id, job_start_datetime_15_min_interval,
	max(max_workers) as max_workers, sum(max_workers) as total_workers, CAST(avg(max_workers) as UNSIGNED) as avg_workers
...
INSERT INTO databricks_cost_analysis (job_run_name, node_type_id, job_start_time_dt, diff, diff_round_up, job_start_datetime_15_min_interval, max_workers)
SELECT jr.job_run_name, c.node_type_id,
    jr.job_start_time_dt,
    TIMESTAMPDIFF(MINUTE, @begin_datetime, jr.job_start_time_dt) AS diff,
    CEILING(TIMESTAMPDIFF(MINUTE, @begin_datetime, jr.job_start_time_dt) / @interval_mins) * @interval_mins AS diff_round_up,
...
(
	...
	from
	(
		SELECT * FROM databricks_cost_analysis
	)15_mins_job_runs
	group by node_type_id, job_start_datetime_15_min_interval
	order by job_start_datetime_15_min_interval
) 15_mins_job_runs_summary
...
SELECT c.node_type_id, jr.job_run_name,
...
(sum(c.max_workers * sp.hourly_price * (jr.job_setup_duration+jr.job_execution_duration)) / datediff(max(jr.job_end_time_dt), min(jr.job_start_time_dt))) as avg_daily_run_cost,
...
sum(c.max_workers * sp.hourly_price * (jr.job_setup_duration+jr.job_execution_duration)) as total_cost
FROM databricks_jobs_run jr
inner join databricks_clusters c on c.cluster_id = jr.job_cluster_id
inner join databricks_sku_price sp on c.node_type_id = sp.sku
group by c.node_type_id, jr.job_run_name;
